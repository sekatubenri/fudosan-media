...
const CATEGORY_STYLES: Record<string, { bg: string; icon: string; iconBg: string }> = {
  mansion: { bg: 'from-blue-600 to-indigo-700', icon: '🏢', iconBg: 'bg-white/20' },
  reit: { bg: 'from-teal-600 to-cyan-700', icon: '📊', iconBg: 'bg-white/20' },
  land: { bg: 'from-emerald-600 to-green-700', icon: '🏠', iconBg: 'bg-white/20' },
  beginner: { bg: 'from-orange-500 to-amber-600', icon: '🌱', iconBg: 'bg-white/20' },
}

export default function ArticleCard({ article }: { article: Article }) {
  const style = CATEGORY_STYLES[article.category] || CATEGORY_STYLES.mansion
